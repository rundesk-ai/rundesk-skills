...
- The [Principles behind the Agile Manifesto](https://agilemanifesto.org/principles) favors
  simplicity—the amount of work not done—and frequent delivery of working software. Applied here,
  the workflow adds planning, agents, and validation surfaces only when the outcome or observed risk
  requires them.
...
- Anthropic's engineering report, [How we built our multi-agent research system](https://www.anthropic.com/engineering/multi-agent-research-system),
  ...
  poorly when work has many dependencies. This field report supports bounded Scout-style discovery
...
- Requiring the primary owner to approve scope, integrate returns, and retain GitHub delivery is an
  operating boundary for accountable handoffs. The sources support small increments, explicit
  authority, and inspected evidence; they do not name those roles or tools.
