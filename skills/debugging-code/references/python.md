...
- **The mutable-default and late-binding traps produce wrong values with no error**, which reads as
  "the logic is wrong" rather than "the language did something." Check `python-patterns`'
...
- `python-patterns` in this catalog, especially `documented-traps.md`, for the failures behind these symptoms
