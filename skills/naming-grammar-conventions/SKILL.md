...
|---|---|---|
| Column header | `How a lead is sold` | `Max buyers` |
| Field label | `Why is this suppressed?` | `Reason` |
| DB column | `sell_type_flag_2` | `max_buyers` |
| Function | `processData()` | `normalizeAddress()` |
| Class | `CarrierHelper` | `CarrierRateSheet` |
| API error | `ERR_5` | `CARRIER_NAME_TAKEN` |
| Event | `carrierUpdateHappened` | `carrier.archived` |
...
`How a lead is sold` → `Max buyers`. `handleUserStuff()` → `deactivateUser()`. The name describes the process rather than identifying the thing.
...
`Nobody has asked to be left alone` → `No suppressed consumers.` `// this is where we cleverly loop back around and fix the thing` → say why, in one line, or delete it.
...
A product with recurring domain terms keeps a **lexicon** in the repository: one canonical term per concept, its layer forms, its definition, and the synonyms to avoid. The lexicon is the artifact that maintains the product vocabulary. See `references/lexicon.md`.
...
| Type or class in the name (`strName`, `reason_text`, `CarrierObject`, `user_list`) | The meaning (`name`, `reason`, `Carrier`, `users`) | The type is already declared, and it changes |
...
| File | Covers |
|---|---|
| `references/code.md` | Variables, booleans, collections, functions, classes, modules, parameters, constants, errors, events, jobs, feature flags, config and env vars, tests, comments, commits |
| `references/data-and-contracts.md` | Tables, columns, timestamps, foreign keys, enums, nullability, indexes, migrations, JSON, REST paths, API fields, error codes, webhooks, logs |
| `references/product-ui.md` | Every user-facing slot: titles, navigation, columns, labels, placeholders, controls, buttons, tooltips, empty states, validation, errors, confirmations, metrics, charts |
| `references/lexicon.md` | How to build and maintain the product vocabulary this guidance depends on |
| `references/sources.md` | Evidence and ecosystem boundaries for the naming rules |
