...
Name timestamps after the event they record. `received_at` and `created_at` are different facts even
when they happen close together.

Name role-bearing references by role when one record points to the same entity more than once:
`created_by_id` and `approved_by_id`, not `user_id` and `user_id_2`.

## States and absence

Use stable machine values for enums and map them to display text. Do not store localized labels as
state. Define the meaning of `NULL`, an empty string, zero, and omitted fields; they are not
interchangeable.

Avoid numeric or string sentinels that overload a legitimate value. Use an explicit nullable value,
state, or companion fact when the domain needs `unknown`, `not applicable`, or `unlimited`.
...
- Separate stable error codes from human messages.
- Treat event payload names and log fields as contracts when consumers query them.
