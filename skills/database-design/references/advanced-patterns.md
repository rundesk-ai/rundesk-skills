...
Index syntax and planner behaviour belong to `postgres-patterns`, `mysql-patterns`, and
`sqlite-patterns`. The indexes shown here exist to say *which access path the model requires*, not
...
cooperates — see `postgres-patterns`, `mysql-patterns`, or `sqlite-patterns` for what your engine
requires.
...
`mysql-patterns` for the generated-column and index syntax.
...
The mechanics of applying a change without downtime are engine-specific — `mysql-patterns` for
online DDL, `sqlite-patterns` for the table rebuild, `postgres-patterns` for which DDL takes which
