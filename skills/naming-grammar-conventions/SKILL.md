...
Read [code naming](references/code.md) for values, collections, functions, types, and rename checks.
Read [data and contracts](references/data-and-contracts.md) for naming stored and published facts
after the data model is chosen. Read [product interface text](references/product-ui.md) for labels,
controls, accessible names, errors, empty states, and feedback. Use `database-design` instead when
choosing the schema model or an ecosystem-specific database convention.

Read [the source map](references/sources.md) when auditing, changing, or extending a lesson in this
package; it states which recommendations are source-backed, ecosystem-specific, or local conclusions.
