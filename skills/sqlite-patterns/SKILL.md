...
description: Use this skill when designing, querying, migrating, tuning, backing up, or diagnosing an application that stores data in SQLite, including lock contention, WAL, foreign keys, type affinity, indexes, FTS5, corruption checks, or live-file handling. It supplies engine-specific decisions and safety rules for SQLite's file-backed, dynamically typed, single-writer model. Do not use it for generic data modelling or a client/server database.
---
