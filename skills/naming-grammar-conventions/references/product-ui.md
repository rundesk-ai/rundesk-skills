...
- `Max buyers`, not `Max Buyers`
- `Add product line`, not `Add Product Line`
...
- Percentages, ratios, and rates get their basis stated: `Conversion rate (leads to sales)`.

### Abbreviations and acronyms
Expand unfamiliar abbreviations on first use when space and context allow, then abbreviate: `National Association of Insurance Commissioners (NAIC)`. Preserve widely recognized domain abbreviations. Do not invent one merely to shorten a label.

### Interpolation and pluralization
- Never concatenate sentence fragments. Build whole strings with placeholders: `{count} carriers selected`, not `"" + count + " carriers selected"`.
...
- Icon-only buttons: the accessible name is the verb plus object the icon performs. A trash icon is `Delete carrier`, not `Trash`, not `Button`.
...
| Good | Bad |
|---|---|
| Suppressions | Manage your suppression list |
| Invoices | Invoice management |
| Progressive Insurance | Carrier detail view |

The definition of the concept does not go here. It goes in the page tooltip.

### Navigation item
**Form:** identical string to the page title it leads to. Never a variant, never a verb.
...
### Breadcrumb
**Form:** the chain of lexicon nouns and record names. `Carriers / Progressive / Product lines`. Never restate the level type.
...
| Good | Bad |
|---|---|
| Details | Basic information about this carrier |
| Product lines | What lines does this carrier write? |
| Notes | Anything else? |
...
> **Suppressions.** Consumers who must not be contacted. Any lead matching a suppressed consumer is blocked before distribution.
...
> **Max buyers.** How many buyers this lead can be sold to. 1 sells it exclusively.
...
| Good | Bad | Why |
|---|---|---|
| Max buyers | How a lead is sold | Describes the mechanism |
...
- Never render a raw stored value (`PENDING_REVIEW`, `product_line`) to a user.
...
### Chart labels
- Chart title: noun phrase naming what is plotted, with the period. `Leads by carrier (last 90 days)`.
- Axis label: the measure and its unit. `Leads`, `Response time (ms)`.
...
**Form:** `{n} {noun}`, correctly pluralized. `1 carrier`, `12 carriers`.

Never `Showing 12 of 12 results`. Never `You have 12 carriers`. When pagination genuinely needs a range, state it as data: `1–50 of 213`.

### Audit and activity log entries
**Form:** `{subject} {past-tense verb} {object}`, with the actor named and the timestamp separate.

> `Jane Doe archived Progressive` · `Mar 4, 2026, 2:14 PM EST`

Never `This carrier was archived by someone` and never a first-person system voice.
...
| Good | Bad |
|---|---|
| Reason | Why is this consumer being suppressed? |
| Requester | Who asked to be suppressed |
| Max buyers | How should this lead be sold? |
| Product lines | Which product lines does this carrier write? |
...
| Good | Bad |
|---|---|
| jane@example.com | Enter the consumer's email address |
| 3 | How many buyers? |
| (nothing) | Type here |
...
|---|---|---|
| Integer 1–8, default 3 | Number input with min and max | Toggle (`Exclusive / Shared`) plus a hidden count field |
| One of three exclusive states | Radio group or select | Two toggles |
...
### Search input
**Form:** placeholder names the searchable fields. `Search by name or NAIC code`. Not `Search...`, not `Find a carrier`.
...
| Good | Bad | Why |
|---|---|---|
| Add consumer to list | Suppress a consumer | Abstract domain verb the user must decode |
| Save carrier | Submit | Does not name the object |
| Add product line | Create new | Does not name the object |
| Send invoice | Confirm | Names the interaction, not the act |

If the button opens a form rather than performing the act, it still names the destination act. `Add carrier` opening a modal is correct; `Open carrier form` is not.
...
### Bulk action
**Form:** verb plus count plus pluralized object, with the count live against the selection. `Delete 4 carriers`.

### Destructive action
**Form:** the real verb plus the object. `Delete carrier`, not `Remove`, not `Clear`, not `Are you sure?` The confirm button inside the dialog repeats the same verb and object.

### Disabled control
**Never disable a control without a tooltip stating the condition that would enable it.**

> Cannot archive a carrier with active product lines.
...
| Good | Bad |
|---|---|
| No consumers in list. | Nobody has asked to be left alone |
| No carriers yet. | It's quiet in here! |
...
> No carriers match the current filters. **Clear filters**
> No results for "progresive". Check the spelling or clear the search.
...
| Good | Bad |
|---|---|
| Max buyers must be between 1 and 8. | Invalid value |
...
| Good | Bad |
|---|---|
| A carrier with that name already exists. | Something here already goes by that name |
| That consumer is already suppressed. | Already added |

### System error
**Form:** state what failed, state whether data was saved, state what to do, give a reference.

> Could not save the carrier. No changes were made. Try again, or contact support with reference `4f21c8`.

Never `Oops`, never `Something went wrong`, never apologize on the system's behalf. Users need to know the state of their data far more than they need sympathy.
...
**Form:** past-tense verb plus object. `Carrier saved.` `Consumer added to list.` Never `Success!`, never `All set!`, never an exclamation mark.
...
### Confirmation dialog
- **Title:** the action as a question, using the real verb. `Delete this carrier?`
- **Body:** the irreversible consequence in one sentence, naming what else is affected. `4 product lines will be removed. This cannot be undone.`
- **Confirm button:** repeats the verb and object. `Delete carrier`. Never `OK`, never `Yes`.
...
### Undo affordance
**Form:** the past-tense confirmation plus `Undo`. `Carrier archived. **Undo**`

### Permission message
**Form:** state the requirement, not the denial. `Requires the Carriers Admin role.` Not `You don't have permission to do that.`
...
- **API error messages and codes:** codes are stable, uppercase, and specific (`CARRIER_NAME_TAKEN`). Messages follow the conflict-error grammar. Never return an internal exception string to a client.
