...
The application's established naming, helpers, constructor style, and file placement take
precedence over this reference. Laravel permits several valid styles; do not introduce a second
dialect for cosmetic preference. Run the repository's configured formatter, commonly Pint.
...
Prefer the shorter form only when it remains clearer in the surrounding code. Request magic
properties and service-location helpers are not automatic improvements.
...
Laravel provides `Str`, `Arr`, `Number`, and `Uri` helper classes that can be more readable,
chainable, and UTF-8 safe than raw PHP functions.

Use these helpers when they clarify semantics or provide needed UTF-8, formatting, or URI behavior.
Do not mechanically replace clear PHP, and verify newer helpers against the installed version.
...
`Arr::get()` parses dots as paths. Index the owning array directly when a literal key itself contains
a dot.
...
Follow the application's established asset and component boundary. Keep substantial page behavior
out of scattered inline JS/CSS, and do not put HTML in domain PHP classes.
...
Comment non-obvious constraints and reasons that names cannot express. Avoid comments that only
narrate the syntax; configuration files are not the only valid place for useful rationale.
