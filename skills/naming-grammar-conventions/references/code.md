...
| Don't | Do |
|---|---|
| `data`, `info`, `result`, `obj`, `tmp`, `val` | `carriers`, `rateSheet`, `normalizedAddress` |
| `d`, `cr`, `prdLn` | `duration`, `carrier`, `productLine` |
| `userList`, `carrierArray`, `strName` | `users`, `carriers`, `name` |
| `data2`, `carriers_new`, `finalFinalTotal` | Distinct meanings get distinct names |
| `timeout`, `size`, `price` (units unstated) | `timeoutMs`, `sizeBytes`, `priceCents` |
| `flag`, `check`, `status2` | `isSuppressed`, `hasConsent`, `reviewStatus` |
...
**Don't** restate the enclosing context: inside a `Carrier` class, the field is `name`, not `carrierName`. Across the boundary, in a function taking many objects, `carrierName` is correct. Name for the reader at that point in the code.
...
| `flag`, `check`, `status` (holding a bool) | `hasConsent`, `isSuppressed` | Names nothing |
...
**Form:** plural of the element noun. `carriers`, `productLines`.

| Don't | Do |
|---|---|
| `carrierList`, `carrierArray`, `carrierCollection` | `carriers` |
| `carrierMap` | `carriersById`, `ratesByProductLine` |
| `items`, `records`, `rows`, `entries` | The element's actual noun |
| `carriersData` | `carriers` |

**Do** name a map by its key and value: `carriersById` reads correctly at the call site (`carriersById[id]`). `carrierMap` does not.

**Do** name a count `carrierCount` or `totalCarriers`, never `carriers` doing double duty.
...
| Don't | Do | Why |
|---|---|---|
| `processData()`, `handleThing()`, `doWork()` | `normalizeAddress()`, `archiveCarrier()` | Names the fact that code runs, not what it accomplishes |
| `manageCarriers()` | Split into the actual operations | "Manage" means the function does several things |
| `getCarriersAndSendEmails()` | Two functions | `and` in a name is the function admitting it does two jobs |
...
**Do** make predicates read as an assertion at the call site: `if (carrier.isEligible())`, not `if (carrier.eligibility())`.

**Do** name a function for its outcome, not its implementation. `fetchCarriersFromRedisThenFallbackToPostgres()` is a comment wearing a function's clothes. `getCarriers()` is the name; the caching strategy is an implementation detail that will change.
...
| `on` / `handle` | Reacts to a named event. Only valid when bound to a specific event: `onCarrierArchived`, never `handleData`. |
...
| Don't | Do | Why |
|---|---|---|
| `CarrierHelper`, `CarrierUtils`, `CarrierManager` | `CarrierRateSheet`, `CarrierArchiver`, `CarrierRepository` | These suffixes mean "assorted functions about carriers." They accumulate until nobody can describe the class. |
| `CarrierData`, `CarrierInfo`, `CarrierObject` | `Carrier` | The suffix adds nothing |
...
| `IManager`, `AbstractBaseCarrierFactoryImpl` | Name the thing | Ceremony is not meaning |
...
**Do** name DTOs and view models after their purpose and shape: `CarrierListItem`, `CarrierCreateRequest`. Not `CarrierDto2`.
...
| `CarrierController.php` containing product-line logic | Keep the file's contents matching its name |
...
| Don't | Do |
|---|---|
| `func(str, num, bool)` | `func(name, maxBuyers, isActive)` |
| `save(carrier, true, false)` | `save(carrier, { validate: true, notify: false })` |
| `copy(a, b)` | `copy(source, destination)` |

**Avoid ambiguous bare booleans.** When `save(carrier, true)` does not explain the condition at the call site, use named options, an enum, or two functions.
...
| Don't | Do |
|---|---|
| `THREE = 3` | `DEFAULT_MAX_BUYERS = 3` |
| `if (status === 2)` | `if (status === CarrierStatus.Archived)` |
...
| Don't | Do |
|---|---|
| `Error`, `AppError`, `CustomException` | `CarrierNameTakenError`, `RateSheetExpiredError` |
| `throw new Error('failed')` | A typed error carrying the entity and condition |
| Messages with pronouns: `it already exists` | `A carrier with that name already exists.` |
...
| Kind | Form | Example |
|---|---|---|
| **Event** (something happened, past) | `{entity}.{past-tense verb}` | `carrier.archived`, `invoice.paid` |
| **Command** (do this, imperative) | `{Verb}{Object}` | `ArchiveCarrier`, `SendInvoice` |
| **Job / worker** (the runner) | `{Verb}{Object}Job` | `SendInvoiceJob` |
| **Handler** (reacts to an event) | `on{Event}` or `handle{Event}` | `onCarrierArchived` |

| Don't | Do |
|---|---|
| `carrierUpdate`, `CarrierEvent`, `carrier.change` | `carrier.archived`, `carrier.rates_updated` |
| An event named as a command (`carrier.archive`) | Events are past tense. If it has not happened yet, it is a command. |
| `ProcessQueueJob` | Name the work: `ReconcileCarrierRatesJob` |
...
| Don't | Do |
|---|---|
| `new_ui`, `flag_2`, `temp_fix` | `carrier_split_form`, `suppression_bulk_import` |
...
| Don't | Do |
|---|---|
| `TIMEOUT` | `CARRIER_API_TIMEOUT_SECONDS` |
...
| Don't | Do |
|---|---|
| `testCarrier()`, `test1()`, `itWorks()` | `archivingCarrierRemovesItsProductLines()` |
| `testSaveMethod()` | `savingCarrierWithDuplicateNameReturnsConflict()` |
...
| `// TODO: fix this` | `// TODO(tim, INV-2431): remove after the rate migration ships` |
...
| Don't | Do |
|---|---|
| `fix`, `updates`, `wip`, `changes per feedback` | `Rename why column to reason on suppressions` |
| `Fixed the thing that was broken` | `Fix empty state shown when carrier filters are cleared` |
...
**Branch:** follow the repository's branch convention. A ticketed repository might use `{ticket}-{short-slug}`, such as `INV-2431-suppression-copy`.
...
**A wrong name is worse than a vague one.** `data` tells a reader nothing; `carrierEmail` holding a phone number tells them something false, and they will act on it.
